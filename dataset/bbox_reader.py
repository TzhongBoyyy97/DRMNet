...
class BboxReader(Dataset):
    def __init__(self, data_dir, set_name, annos, cfg, max_num, mode='train'):
        self.mode = mode
        self.cfg = cfg
...
#             self.filenames = f.read().splitlines()
        self.filenames = np.load(set_name,allow_pickle=True)
        if mode != 'test':
...
#             import pdb;pdb.set_trace()
            if len(self.bboxes) < max_num:
                size = max_num - len(self.bboxes)
                random_list = np.random.randint(len(self.bboxes), size=size)
                extend_list = [self.bboxes[ran_num] for ran_num in random_list]
                self.bboxes.extend(extend_list)
    ...
    def __getitem__(self, idx):
        ...
        if self.mode in ['train', 'val']:
            if not is_random_img:
                ...
                imgs = np.load(os.path.join(self.data_dir, '%s_clean.npy' % (filename)))
...
#                 imgs = np.expand_dims(imgs,0)
            ...
            # np.save(os.path.join('./crops',filename+'_sample.npy'),sample)
            # np.save(os.path.join('./crops',filename+'_bboxes.npy'),bboxes)
        ...
        if self.mode in ['eval']:
            ...
            image = np.load(os.path.join(self.data_dir, '%s_clean.npy' % (filename)))
#             image = np.expand_dims(image,0)
            ...
            if 'PN9' in self.set_name:
                if image.shape[2]>400:
                    crop_size = [20,110,70]
                else:
                    crop_size = [20,50,50]
                image = image[:,crop_size[0]:-crop_size[0],crop_size[1]:-crop_size[1],crop_size[2]:-crop_size[2]]#predefine
            else:
                crop_size = [0,0,0]
...
#             truth_bboxes = bboxes[:, :-1]
            ...
            return [torch.from_numpy(input).float(), truth_bboxes, truth_labels, crop_size]
