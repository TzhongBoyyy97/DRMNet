...
def make_rcnn_target(cfg, mode, inputs, proposals, truth_boxes, truth_labels):
    ...
    for b in range(batch_size):
        try:
            ...
            print(truth_labels[b], type(truth_labels[b]))
            truth_boxes [b] = []
            truth_labels[b] = []
...
#         truth_labels_temp = []
# #             index = np.where(truth_labels[b] > 0)[0]
#         for index,item in enumerate(truth_labels[b]):
#             if item>0:
# #                 indexes.append(index)
#                 truth_boxes_temp.append(truth_boxes[b][index])
#                 truth_labels_temp.append(truth_labels[b][index])
#         truth_boxes[b] = np.array(truth_boxes_temp)
#         truth_labels[b] = np.array(truth_labels_temp)
    ...
    for b in range(batch_size):
        input = inputs[b]
        truth_box   = np.array(list(truth_boxes[b]), dtype=np.float64)
        truth_label   = np.array(list(truth_labels[b]), dtype=np.float64)
