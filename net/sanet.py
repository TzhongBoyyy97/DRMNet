...
class FeatureNet(nn.Module):
    ...
    def forward(self, x):
        ...
        B, C = rev3.shape[:2]
        n_tokens = rev3.shape[2:].numel()
        img_dims = rev3.shape[2:]
        rev3_feature_flat = rev3.view(B, C, n_tokens).transpose(-1, -2)
        rev3_feature_flat = self.ln1(rev3_feature_flat)
        rev3 = self.mamba1(rev3_feature_flat)
        rev3 = rev3.transpose(-1, -2).view(B, C, *img_dims)
        ...
        B, C = rev2.shape[:2]
        n_tokens = rev2.shape[2:].numel()
        img_dims = rev2.shape[2:]
        rev2_feature_flat = rev2.view(B, C, n_tokens).transpose(-1, -2)
        rev2_feature_flat = self.ln1(rev2_feature_flat)
        rev2 = self.mamba1(rev2_feature_flat)
        rev2 = rev2.transpose(-1, -2).view(B, C, *img_dims)
    ...
    def forward(self, f, inputs, proposals):
        ...
        for p in proposals:
            ...
            try:
                ...
                c0 = np.array([z_start.item(), y_start.item(), x_start.item()])
                c1 = np.array([z_end.item(), y_end.item(), x_end.item()])
...
class SANet(nn.Module):
    def __init__(self, cfg, mode='train'):
        ...
        self.use_rcnn = False
         #lddg
        self.feature_dim = cfg['latent_dim']
        self.mu = nn.Sequential(nn.Conv3d(128, 128, kernel_size=1), nn.ReLU(),
                                nn.Conv3d(128, self.feature_dim, kernel_size=1))
        self.logvar = nn.Sequential(nn.Conv3d(128, 128, kernel_size=1), nn.ReLU(),
                                    nn.Conv3d(128, self.feature_dim, kernel_size=1))
    ...
    def forward(self, inputs, truth_boxes, truth_labels, split_combiner=None, nzhw=None):
        ...
        if self.mode in ['train', 'valid']:
            ...
            self.mu_logvar = torch.stack([mu, logvar, feature], dim=1)
        else:
            feature = mu
            # logits = self.outc(feature)
            # return logits, mu
        ...
        feature = self.mu_logvar[:, 2].permute(0, 2, 3, 4, 1).contiguous().view(-1, self.feature_dim)
        feature = feature[torch.randperm(len(feature))]
        U, S_spinal, V = torch.svd(feature[0:2000])
        self.low_rank_loss_spinal = S_spinal[2]
...
#                 rcnn_crops = self.rcnn_crop(features, inputs, torch.from_numpy(proposal).cuda())
    ...
    def loss(self, targets=None):
        ...
        #lddg
        self.loss_kl_spinal = kl_gaussian_loss(self.mu_logvar[:, 0], self.mu_logvar[:, 1])
    
        self.detect_loss = self.rpn_cls_loss + self.rpn_reg_loss \
                          + self.rcnn_cls_loss +  self.rcnn_reg_loss
        self.total_loss = self.detect_loss + self.loss_kl_spinal * cfg['kl_weight'] * 2 + self.low_rank_loss_spinal * cfg['low_rank_tradeoff']
        # self.total_loss = self.detect_loss + self.loss_kl_spinal * cfg['kl_weight'] * 2
